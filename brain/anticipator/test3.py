...
HWState = namedtuple('HWState', 't level trend season_period')
...
def advance(y, params, state):
    ...
    # in a proper implementation, we would enforce season_period being 0-mean.
...
def hw(data, split=None, params=None):
    ...
    print("||season_period|| = {:.3f}".format(np.sqrt(np.sum(state.seasons ** 2))))
    if params is None:
    ...
    print("final ||season_period|| = {:.3f}".format(np.sqrt(np.sum(state.seasons ** 2))))
    return fcast, level
